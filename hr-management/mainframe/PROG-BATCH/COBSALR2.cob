      *-----------------------------------------------------------------
      *    COBSALR2 - SAMPLE COBOL PROGRAM TO DEMONSTRATE CICS CALLS
      *                                                                 
      *    FUNCTIONALITY - READS EMPLOYEE DATA FROM DATABASE. THIS PROGRAM    
      *                    FETCHS DETAILS OF TOP 10 EMPLOYEES HAVING SALARY 
      *                    IN THE SPECIFIED RANGE. PROGRAM TAKES SALARY AND 
      *                    ALLOWED VARIATION AS INPUT AND CHECKS FOR THE 
      *                    EMPLOYEES HAVING SALARY IN THE SPECIFIED RANGE.
      *                                                                 
      *-----------------------------------------------------------------
      *                                                                 
      ...
      *-----------------------------------------------------------------
       01  WS-WORKING-VARS.                                                 
               ...
               02  WS-EMP-SALARY-LRANGE    PIC S9(7)V99.                
               02  WS-EMP-SALARY-URANGE    PIC S9(7)V99.                
               02  WS-VARIATION            PIC S9(3).                
               02  WS-COUNTER              PIC S9(2).       
               02  WS-FETCH-COUNT          PIC S9(2) VALUE 1.       
      ...
      *                EMP                                              
      *           WHERE 
      *                SALARY BETWEEN :WS-EMP-SALARY-LRANGE 
      *                           AND :WS-EMP-SALARY-URANGE
      *           ORDER BY 
      *                SALARY
      ...
      * PROCEDURE DIVISION                                                                                                               
      *-----------------------------------------------------------------                                                                  
       PROCEDURE DIVISION USING CASTMSALI, CASTMSALVARI, 
                                CASTMEMPDETAILSO. 
                                                                        
              PERFORM 1000-CALCULATE-VARIATION.                      
      ...
      *-----------------------------------------------------------------                                                                  
       1000-CALCULATE-VARIATION.
       
               MOVE CASTMSALVARI           TO WS-VARIATION


      *        CALCULATE SALARY LOWER BOUND

               MULTIPLY WS-VARIATION       BY WS-SALARY 
                                       GIVING WS-EMP-SALARY-LRANGE

               DIVIDE 100                FROM WS-EMP-SALARY-LRANGE
                                       GIVING WS-EMP-SALARY-LRANGE

               SUBTRACT WS-EMP-SALARY-LRANGE 
                                         FROM WS-SALARY 
                                       GIVING WS-EMP-SALARY-LRANGE
               
           
      *        CALCULATE SALARY UPPER BOUND

               MULTIPLY WS-VARIATION       BY WS-SALARY 
                                       GIVING WS-EMP-SALARY-URANGE

               DIVIDE 100                FROM WS-EMP-SALARY-URANGE
                                       GIVING WS-EMP-SALARY-URANGE

               ADD WS-EMP-SALARY-URANGE 
                                           TO WS-SALARY 
                                       GIVING WS-EMP-SALARY-URANGE
