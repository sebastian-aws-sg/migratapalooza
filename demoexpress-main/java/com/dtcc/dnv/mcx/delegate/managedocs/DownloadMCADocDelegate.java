...
import com.dtcc.dnv.mcx.proxy.managedocs.DownloadMCADocProxy;
...
public class DownloadMCADocDelegate extends MCXAbstractBusinessDelegate
{
    
    private final static MessageLogger log = MessageLogger.getMessageLogger(DownloadMCADocDelegate.class.getName());
    ...
    public IServiceResponse processRequest(IServiceRequest request) throws BusinessException, UserException
    {
        ...
        final String REQUEST_ID = "DPMXDDLD";
        try
        {
            log.info("In DownloadMCADocDelegate");
            ...
            DownloadMCADocProxy dbProxy = new DownloadMCADocProxy();
            ...
            log.info("Returning from DownloadMCADocDelegate");
        } catch (DBException dbe)
        {
            log.error("DBException in DownloadMCADocDelegate.processRequest() method ", dbe);
            throw new BusinessException(dbe.getSqlCode(), dbe.getSqleMessage());
        } catch (Exception e)
        {
            log.error("General Exception in DownloadMCADocDelegate.processRequest() method ", e);
            throw new BusinessException("DownloadMCADocDelegate", "Error occurred building database request.");
        }
