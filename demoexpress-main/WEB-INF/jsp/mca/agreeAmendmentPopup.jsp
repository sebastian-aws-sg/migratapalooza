<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean"%>
<head>
<title>Agree Amendment</title>
</head>
<body onload="notEmpty()">
<script type="text/javascript">
function notEmpty(){
var obj = window.dialogArguments;
var termValueId = obj.termValueId;
var lockSt = obj.lockSt;
var tmpltType = obj.tmpltType;
var tmpltID = obj.tmpltID;
var isDealer = obj.isDealer;
var randomnumber=Math.floor(Math.random()*100001)
document.getElementById("agreeAmend").src = "/mcx/action/agreeAmendment?tmpltID="+tmpltID+"&termValId="+termValueId+"&tmpltType="+tmpltType+"&tmpltLocked="+lockSt+"&actingDealer="+isDealer+"&random="+randomnumber;
}
</script>
<form>
<iframe id="agreeAmend" src="javascript:'';"
width="100%" height="100%" name="popup" leftmargin=0  framespacing=0  frameborder=no border=0  marginheight=0 marginwidth=0>

</iframe>
</form>
</body>
